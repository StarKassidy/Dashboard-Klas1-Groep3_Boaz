...
# Assuming df_liveweer is already loaded and contains 'plaats', 'temp', and 'samenv' columns

# City coordinates for icon placements
city_coords = {
    "Assen": [52.9929, 6.5642],
    "Lelystad": [52.5185, 5.4714],
    "Leeuwarden": [53.2012, 5.7999],
...
# Streamlit dropdown menu to select visualization type
visualization_option = st.selectbox("Select the visualization type", ["Temperature", "Weather"])

# Create a folium map centered over the Netherlands
nl_map = folium.Map(location=[52.3, 5.3], zoom_start=8)

# Add markers based on selected visualization option
for index, row in df_liveweer.iterrows():
    if visualization_option == "Weather":
        weather_desc = row['samenv'].lower()
        icon_file = "bewolkt.png"  # Default icon for weather
        icon_path = f"iconen-weerlive/{icon_file}"  # Folder path updated
